...
server.tool(
  "record_video",
  "Record a browser session as video. Three engines: (1) DEFAULT 'ffmpeg-native' captures the OS screen directly via ffmpeg in the native host (gdigrab on Windows, avfoundation on macOS, x11grab on Linux). No Chrome user-gesture requirement, no permission picker, fully automatable. Window must be visible (un-minimized) during recording. Output: real 30fps mp4/webm/gif encoded straight to disk. (2) 'media-recorder' uses chrome.tabCapture + MediaRecorder via an offscreen document - frame-paced webm/VP9. Requires the user to click the Orellius toolbar icon once (Chrome's activeTab security gate) OR shows a screen-picker via getDisplayMedia. Useful when you need to record content the OS can't see (DRM-protected video, off-screen tabs). (3) 'cdp-legacy' uses CDP Page.startScreencast - paint-event-driven, frames only emitted when the page repaints. Sparse/choppy on static pages, but includes synthetic cursor + click ripple overlays composited per-frame. Workflow: 1) start_recording, 2) drive the page via other tools, 3) stop_recording (ffmpeg-native auto-finalizes, others need export). All operations scoped to a tab.",
  {
    ...
    options: z.object({
      engine: z.enum(["ffmpeg-native", "media-recorder", "cdp-legacy"]).optional().describe("Recording engine. Default 'ffmpeg-native' (no gesture, captures OS screen, but the window MUST be visible/un-minimized or it records black). Use 'media-recorder' for the most RELIABLE unattended capture: with the Chrome launch flags --auto-accept-this-tab-capture + --use-fake-ui-for-media-stream it captures the current tab via getDisplayMedia({preferCurrentTab}) with NO toolbar-icon click and NO picker, and is occlusion-independent (records even when the window is hidden/behind others/on another virtual desktop). Without those flags media-recorder falls back to the picker. A liveness check on start now warns if no frames flow in the first ~1.5s instead of silently producing an empty file. Engine locked at start_recording; stop/export/clear auto-detect."),
      // ffmpeg-native engine options (also shared by media-recorder for frameRate/videoBitsPerSecond)
      frameRate: z.number().optional().describe("[ffmpeg-native, media-recorder] Target frame rate, 5-60 (default 30). Real video timeline regardless of page paint events."),
      videoBitsPerSecond: z.number().optional().describe("[ffmpeg-native, media-recorder] Encoder bitrate (default: ffmpeg-native uses CRF 23, media-recorder uses 2.5 Mbps)."),
      drawCursor: z.boolean().optional().describe("[ffmpeg-native] Capture the OS cursor in the recording (default true)."),
      windowTitle: z.string().optional().describe("[ffmpeg-native] Specific window title to capture (default: the tab's current title). Pass a substring to match if the auto-detected title is fragile."),
      region: z.object({ x: z.number(), y: z.number(), width: z.number(), height: z.number() }).optional().describe("[ffmpeg-native] Capture a specific screen region instead of the window. Overrides windowTitle."),
      ...
      captureMode: z.enum(["auto", "tab-capture", "display-media", "cdp"]).optional().describe("[media-recorder] Capture strategy. 'auto' (default) tries the silent tab-capture path, then falls back to the HANDS-FREE 'cdp' path. 'tab-capture' silent only (errors if the extension was not invoked via a toolbar-icon click). 'display-media' picker only (auto-accepts with the --auto-accept-this-tab-capture launch flags). 'cdp' = fully hands-free tab capture with NO toolbar click, NO launch flags, NO picker: the extension's debugger runs Page.startScreencast and pipes frames into an offscreen canvas backing a MediaRecorder. Works on any Chrome the extension is loaded in; the tab must be visible/foregrounded while recording (hidden tabs don't paint)."),
